...
export const HeroBanner = () => {
 ...
 const styleByLayoutMain: Record<string, any> = {
    A: {
      ...
      image:
        "w-[25vw] h-[65vh]  object-cover brightness-75 hover:brightness-90 transition-transform duration-700 rounded-3xl shadow-2xl",
    ...
    B: {
      ...
      image:
        "w-[25vw] h-[65vh]  object-cover rounded-3xl shadow-2xl hover:scale-105 transition-transform duration-700",
      text: "w-[45%] flex flex-col gap-4 text-gray-900",
    ...
    E: {
      wrapper:
        "relative overflow-hidden h-[90vh] flex flex-col items-center justify-center bg-transp text-white rounded-[3rem] ",
      image:
        ...
        >
          ...
          >
            ...
            {data.h1 && (
              <motion.h2
                variants={fadeUp(0.2)}
                className={`text-xl md:text-2xl mb-4 ${data.colorText || "text-gray-500"}`}
              >
                {data.h1}
              </motion.h2>
            )}
        ...
        {banners.map((_, i) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              i === index ? "bg-gray-800 scale-125" : "bg-gray-400/50 hover:bg-gray-600"
            }`}
          />
        ))}
      </div>
